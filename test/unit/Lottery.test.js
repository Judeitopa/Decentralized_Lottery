...
!developmentChains.includes(network.name)
    ...
    : describe("Lottery Unit Tests", function () {
          let lottery, raffleContract, vrfCoordinatorV2Mock, raffleEntranceFee, interval, player // , deployer
          ...
          beforeEach(async () => {
              ...
              raffleContract = await ethers.getContract("Lottery") // Returns a new connection to the Raffle contract
              lottery = raffleContract.connect(player) // Returns a new instance of the Raffle contract connected to player
          ...
          describe("enterRaffle", function () {
              ...
              it("emits event on enter", async () => {
                  await expect(lottery.enterRaffle({ value: lotteryEntranceFee })).to.emit( // emits RaffleEnter event if entered to index player(s) address
                      lottery,
                      "RaffleEnter"
                  )
              })
          ...
          describe("checkUpkeep", function () {
              ...
              it("returns false if raffle isn't open", async () => {
                  ...
                  const raffleState = await lottery.getRaffleState() // stores the new state
                  const { upkeepNeeded } = await lottery.callStatic.checkUpkeep("0x") // upkeepNeeded = (timePassed && isOpen && hasBalance && hasPlayers)
            ...
            // All the assertions are done once the WinnerPicked event is fired
              it("picks a winner, resets, and sends money", async () => {
                  ...
                  for (let i = startingIndex; i < startingIndex + additionalEntrances; i++) { // i = 2; i < 5; i=i+1
                      lottery = raffleContract.connect(accounts[i]) // Returns a new instance of the Raffle contract connected to player
                      await lottery.enterRaffle({ value: raffleEntranceFee })
                  }
